...
const TabNavigator = createBottomTabNavigator(
  {
    ...
    VideoChat: {
      ...
      navigationOptions: {
        tabBarLabel: <></>,
        tabBarIcon: ({tintColor}) => (
          <Icon name="videocam" style={{color: tintColor}} size={25} />
        ),
      },
    },
    Chat: {
      ...
      navigationOptions: {
        tabBarLabel: <></>,
        tabBarIcon: ({tintColor}) => (
          <Icon name="chatboxes" style={{color: tintColor}} size={25} />
        ),
      },
    },
    Profile: {
    ...
    NearbyLocations: {
      ...
      navigationOptions: {
        tabBarLabel: <></>,
        tabBarIcon: ({tintColor}) => (
          <Icon name="wine" style={{color: tintColor}} size={25} />
        ),
      },
    },
  },
...
export default class App extends React.Component {
  ...
  makeCall = async user => {
    ...
    this.props.navigation.navigate('VideoChatPack', {
  ...
  componentDidMount = async () => {
    ...
    this.socket = io(baseurl, {
      query: `user_id=${user_id}&username=${username}&user=${JSON.stringify(
        user,
      )}`,
    });
  ...
  messageListener = async () => {
    this.notificationListener = firebase
      ...
      .onNotification(notification => {
        ...
        local.android
          .setChannelId(notification.android.channelId)
    ...
    this.notificationOpenedListener = firebase
      ...
      .onNotificationOpened(async notificationOpen => {
        if (notificationOpen) {
          await this.setState({
            go_to_chat_interface: true,
            notification_object: notificationOpen,
          });
        }
      });
