...
export default class VideoChat extends React.Component {
  ...
  loadMatches = async () => {
    ...
    let result = await getData(
      `like/load-matches-by-profileId/${this.props.screenProps.me.id}`,
    );
    if (result.ok) {
  ...
  render() {
    ...
    const online_users = this.props.screenProps.users.filter(u =>
      matches_ids.includes(parseInt(u.info.id)),
    );
    if (loading) {
      return (
        <View>
    ...
    return (
      <Container>
                ...
                return (
                  <ListItem thumbnail>
                    <Left>
                      <Thumbnail
                        source={{
                          uri: `${baseurl}/user_images/${item.info.profile_pic}`,
                        }}
                      />
                    </Left>
        ...
        {this.state.calling_type === 'audio' &&
        this.state.localVideo &&
        this.state.remoteVideo ? (
          <View
            style={{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
            <Text style={{color: 'white', fontSize: 18}}>
              You Both Are On Audio Call...
            </Text>
          </View>
        ) : null}
        <View
